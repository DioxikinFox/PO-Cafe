...
// --- KAWALAN KEDAI (AUTO BUKA 11AM - 10PM) ---
    ...
    function checkAutoStoreStatus() {
        ...
        getDoc(doc(db, "settings", "storeStatus")).then(docSnap => {
            ...
            // AUTO-TUTUP jika kedai masih buka tapi waktu operasi dah tamat
            ...
            // AUTO-BUKA semula jika waktu operasi dah masuk dan sebab tutupnya adalah "Tutup automatik"
    ...
    checkAutoStoreStatus(); // Jalankan waktu awal mula buka page admin
    setInterval(checkAutoStoreStatus, 60000); // Semak setiap 1 minit
    ...
    if (btnSaveStatus) {
        btnSaveStatus.addEventListener('click', async (e) => {
            ...
            // Pengawalan manual akan abaikan "Tutup automatik" supaya tak bertembung
 ...
 // --- PENGURUSAN PROMOSI (FIX: TAPISAN TARIKH INVALID 'NaN' & ANTI-CRASH) ---
    window.fetchPromotions = function() {
        ...
        onSnapshot(collection(db, "promotions"), (querySnapshot) => {
            ...
            if (querySnapshot.empty) {
                tb.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:20px; color:#888;">Tiada data promosi dijumpai. Sila klik "Import Promosi" atau tambah manual.</td></tr>`;
                return;
            }
            ...
            querySnapshot.forEach((docSnap) => {
                try {
                    ...
                    // Semakan paling KETAT untuk elakkan tarikh luput jadi 'NaN' dan rosakkan jadual
                    if (item.expiryDate && item.expiryDate !== "") {
                        const dateObj = new Date(item.expiryDate);
                        // Semak jika objek tarikh adalah tarikh yang SAH
                    ...
                    const mediaHTML = type === 'video' 
                        ...
                        : `<img src="${src}" width="40" height="40" style="object-fit:cover; border-radius:5px;" onerror="this.src='https://via.placeholder.com/40?text=Imej'">`;
                    ...
                    console.error("Ralat memproses satu data promosi (Jadual tetap akan terus berjalan):", docSnap.id, err);
                }
            });
        }, (error) => {
            tb.innerHTML = `<tr><td colspan="6" style="text-align:center; color:red;">Gagal panggil database: ${error.message}</td></tr>`;
        });
    };
    fetchPromotions();
    // --- BUTANG AUTO IMPORT PROMOSI (CEGAH PENDUA / DUPLICATE) ---
    ...
    // --- BUTANG AUTO IMPORT MENU (LENGKAP DESKRIPSI & ANTI-DUPLICATE) ---
...
// --- MAKLUM BALAS PELANGGAN (DIPERBAIKI TAPISAN) ---
    window.fetchFeedbacksRealtime = function() {
        ...
        onSnapshot(collection(db, "feedbacks"), (querySnapshot) => {
            const renderFeedbacks = () => {
                container.innerHTML = "";
                // Tangkap nilai semasa dropdown atau set "all"
                let selectedStar = filterStarSelect ? filterStarSelect.value : "all";
                let hasData = false;

                querySnapshot.forEach(docSnap => {
                    const data = docSnap.data();
                    // Jika data lama tiada rating, set default 5
                    let rating = 5; 
                    if (data.rating !== undefined && data.rating !== null) {
                        rating = parseInt(data.rating, 10);
                    }

                    // Logik tapisan yang tepat
                    if (selectedStar !== "all" && selectedStar !== "" && parseInt(selectedStar, 10) !== rating) {
                        return; // Langkau jika bintang tak sama dengan pilihan
                    }
